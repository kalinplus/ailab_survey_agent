...
- **A** (`harness/`): `AgentLoop.run()` (agent_loop.py:115-189) is a hardcoded sequence, not LLM-driven. `search_strategy_builder.py` builds sub-queries (3-level fallback: LLM → cluster → template; LLM only in `--mode full`). **`harness/agents/` = 关节1 策略 Agent** (spec `docs/RealAgent/关节1-策略Agent-方案与测试.md`): `loop.py` shared bounded chassis with dual wire modes — default JSON typed-action, or native Chat Completions tool calling via `tool_schemas` + `llm_tool_chat` (endpoint support verified live by `scripts/probe_tool_calling.py`; Intern-S2 + DeepSeek both pass a 3-turn smoke) — plus budget breakers + trajectory jsonl, `relevance.py` report card (bge embedding, keyword fallback), `strategy_agent.py` probe→cluster→naming→sample→edit-loop(≤3 rounds)→deterministic gate→best-so-far rollback. Enabled when `--mode full` (disable via `EVISURVEY_STRATEGY_AGENT=0`); strategy joint native wire via `EVISURVEY_AGENT_TOOLS=1` (default off until a real-run A/B); toolbox = permission boundary; output schema unchanged (P1–P6 untouched). `tool_registry.py` dispatches B/C tools in-process with timeout + result truncation. `--prepare-only` stops after A emits requests.
...
**Known traps (verified, see audit §1 in docs/RealAgent/总计划):** NLI defaults to a keyword FakeNLIModel (real model needs `EVISURVEY_REAL_NLI=1` — hard precondition for repair experiments; **real-NLI runs must also set `HF_HUB_OFFLINE=1`** — models are cached locally, and an HF HEAD connection reset crashes the run); `use_mineru` now opt-in via `--use-mineru` (default False → demo-mode evidence store stays thin, masked only by FINAL_SEED_PAPERS); the old discarded-second-verify half-loop is fixed by `_verify_repair_loop` + Goal Gate; full-mode runs with repair agent can exhaust memory on a 16GB machine during the final verify (silent SIGKILL, no traceback — mitigate with lower `--max-papers`); plotting modules pin `matplotlib.use("Agg")` because ToolRegistry worker threads cannot use the macosx GUI backend. **Test-suite pollution (data-loss grade, fixed cd5b424):** running the full unit suite used to clobber live `cache/*.json` via `write_final_seed_files(Path.cwd())` — archive real-run artifacts BEFORE running the full suite (see `archive/e2e-*`). Goal-gate failures now end as `status=quality_failed` with nonzero CLI exit (was: always `completed`). **Eval-specific (2026-08-21):** output/ workspace mixes artifacts from different runs — `output/survey.md` cites `paper:doi` ids that exist in NO evidence store (must pair it with `cache/eval_evidence_store.json`, built per-claim via SciVerse agentic-search + meta-search abstracts); NLI margin confidences are compressed (identical text ≈0.56 < 0.6 threshold), so `tools/evaluate_survey.py` verdicts follow the NLI **label** (ALCE/AutoSurvey convention) and evidence is split into **sentence windows** (cross-encoder NLI fails on long-passage premises: a verbatim quote inside a 1700-char abstract scores neutral); EN cross-encoder + zh survey → residual neutral (weak) bias; `cache/gold_refs.json` is extracted from the arXiv HTML bibliography (25 malformed entries flagged `malformed:true`, substring matching allowed ONLY for those — a short title like "World models" substring-matching clean titles is a false-positive trap; DreamerV3 is cited in that survey's body but missing from its rendered bib). (Dead `harness/agent_tool_loop.py` was removed when the shared chassis landed in `harness/agents/loop.py`.)
...
EVISURVEY_REAL_NLI=1 python scripts/run_survey_eval.py --evidence-store cache/eval_evidence_store.json  # 事后三层评测（NLI 引用质量 / gold 参考覆盖 / Intern-S2 judge + 修复关节 A/B 表）
...
## Acceptance Discipline (lesson paid for 2026-09-08; see docs/DeepSeek与MinerU输出质量问题及后续方案.md §8)

A batch/direction is **accepted only when both** hold:

1. Its mechanism tests pass (unit + targeted), AND
2. The end artifact passes a **document-level read**: full-text read of `output/survey.md` against an itemized checklist — off-topic papers, repetition/recycled claims, fact-vs-citation scope (does the cited evidence actually say this), readability, leaked internal text. For real-run acceptances also run `scripts/run_survey_eval.py` (L0 redundancy + L1 NLI + **L3 judge** — the only layer that reads the document) on the same run's artifacts; L3 has been skipped since 2026-09-04 and that gap is exactly how off-topic papers and fluent wrong facts survived five batch acceptances.

Hard rules:

- `topic_relevance` below threshold in `review_report.json` must be answered explicitly in the acceptance report (fix / waive / downgrade) — never silently skipped (0.692 sat unread in the latest artifacts).
- Report `citation_validity` as "citation structure legal", never "citations correct"; whitelist membership says nothing about topical fit. Counts are not usability (figure records ≠ usable images; evidence rows ≠ unique evidence — check id uniqueness).
- Metrics share the generator's data chain: cross-side consistency validates parsing, not truth. The whitelist "extra junk" direction has no detector in canonical audit — check it manually until one exists.
- Mechanism acceptance ≠ artifact acceptance. Signing "批次验收通过" on numbers alone, without reading the deliverable, is the failure mode this section exists to prevent.
