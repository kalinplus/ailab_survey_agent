...
> 性质：审计部分为已验证的代码事实（含 file:line 锚点），设计部分为 v2 改造方案。**关节 1/2/3 已全部实施**（见各模块文档），评测模块见 §7。
...
## 7. 评测模块：三层评估与实测结果（2026-08-21 落地）

§6 说"要有对照实验数据"——先得有测量仪器。评测模块就是这个仪器：`tools/evaluate_survey.py`（评测逻辑，纯函数可单测）+ `scripts/run_survey_eval.py`（入口）。**它是事后审计工具，不在 agent_loop 主链路上**——评的是已经写完的 survey，不参与生成。

### 7.1 三层各测什么（说人话）

| 层 | 问题 | 怎么算 |
|---|---|---|
| L1 引用质量 | 综述里的引用是不是真的支撑了那句话 | 把每个带引用的句子按（句子 × 每个引用）拆成一对对，每对用真 NLI 模型判"证据蕴含这句话吗"。三个数：recall（多少句至少有一个引用撑得住）、precision（所有引用实例里撑得住的比例）、coverage（多少比例的句子带引用） |
| L2 参考覆盖 | 系统选的论文，专家认不认 | 拿一篇真实发表的 world models 综述（arXiv 2411.14499，339 条参考文献）当标准答案，看我们语料里的论文有多少在专家名单里、专家名单我们覆盖了多少 |
| L3 LLM 打分 | 读起来怎么样 | Intern-S2 按七个维度（流畅/逻辑/冗余/清晰/无错/覆盖/切题）给每节打 1-5 分 |

另有 A/B 换算表：读 `output/repair_ab_report.json`，把修复关节三个配置（A 不修 / B 删除式 / C 动作修复）的数据换成同一张表，直接展示关节 2 的效果。

### 7.2 实测结果（交付版 survey，真 NLI + 真 Intern-S2）

| 指标 | 数值 | 一句话解读 |
|---|---|---|
| L1 citation recall / precision / coverage | **0.455 / 0.581 / 0.28** | 43 个引用对里 25 个撑得住；撑不住的主要是模板导语句 |
| L2 reference recall | 0.033（11/338） | 12 篇语料对 338 条专家文献，天花板就在这 |
| L2 system-in-gold | **0.583（7/12）** | 语料过半被专家综述背书（World Models、Genie、GameNGen、GAIA-1、MineDojo、Generative Agents、MuZero 类） |
| L3 judge overall | 3.94–4.08（两次运行波动） | 12 节；accuracy 最高、coverage 最低 |
| A/B 修复关节 | claims 61→20→50，invalid 84→0→0，validity 0.47→1.0→1.0 | B 靠删、C 保内容——关节 2 的核心卖点 |

### 7.3 调仪器比跑数字花的功夫大（三个实测过的坑）

数字低的时候先别急着改系统，先验仪器。三个坑都是在实测中暴露、复现、修掉的：

1. **NLI 读不了长文章**。把整段 1700 字摘要当证据喂给它，哪怕引用的那句话一字不差地在摘要里，它也判"无关"。切成一句一句再判，命中句立刻判"蕴含"。修这一条，recall 从 0.152 → 0.455。另外它的置信度是压缩的（完全相同的两句话也只有 0.56），生成侧 0.6 的阈值天然过不了——所以评测器只看判定标签（蕴含/无关/矛盾），不看分数，这也是 AutoSurvey 的口径。
2. **"标准答案"本身是脏的**。Semantic Scholar 的参考文献接口和 arXiv 网页版书目对不上（后者还有 25 条格式损坏、DreamerV3 被正文引用但书目里没这条）。最后从 arXiv HTML 书目直接提取，损坏条目打上标记单独处理。教训：验证 gold 数据时要用论文真实标题当探针，用错探针词会得出"数据源不全"这种看起来合理实际错误的结论。
3. **短标题会乱匹配**。"World models"（Ha & Schmidhuber 的论文标题就这俩词）是 35 个其他论文标题的子串，一度把 reference recall 虚推到 0.124。修复：子串匹配只允许用在格式损坏的条目上，回落到真实的 0.033。

还有一个工程前提：`output/` 里混着不同 run 的产物，交付版 survey 引用的论文 id 在任何 evidence store 里都查不到——评测前必须先确认（survey， evidence）是同一次运行的，为此专门用 SciVerse 给这 3 篇论文现场构建了 `cache/eval_evidence_store.json`。

### 7.4 修完仪器，剩下低的才是真问题

- **L1 的 14 weak + 4 unsupported**：全是"这一部分关注的问题是……"这类没有证据支撑的模板导语句。这是 write_survey 模板化写作的锅，指明了下一步改进方向（导语句要从证据生成，不是填空）。
- **L2 reference recall 0.033**：12 篇语料太小。解药是加大检索预算跑真实全量，这是系统升级项，不是评测问题。
- **L3 coverage 维度最低（3.33）**：与 L2 互相印证——语料小，覆盖自然弱。

### 7.5 怎么重跑

```bash
EVISURVEY_REAL_NLI=1 python scripts/run_survey_eval.py --evidence-store cache/eval_evidence_store.json
# 输出 output/survey_eval_report.json + .md；--skip-judge/--skip-fuzzy 可降级
# 单测：pytest tests/unit/test_evaluate_survey.py -v（34+ 用例，含三个坑的回归测试）
```

### 7.6 下一步（未做，明确列出）

- AHA 对抗审计当裁判（初判 → 质疑 → 辩护 → 终裁），替换单裁判——设计已在报告 md 里标注"in design"；
- 双语 NLI（如 mDeBERTa）或 claim 翻译层，消掉中英混排句的 neutral 偏差；
- 换更大检索预算重跑 L2，把"语料天花板"和"检索能力"分开。
